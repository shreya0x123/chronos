...
async def run_queries_benchmark():
    ...
    if trace_count == 0:
        print("Error: No traces in database. Please run benchmark.py first to populate data!")
        return
    ...
    print(f"Selected trace ID {trace_id} ({target_trace.get('span_count', 1)} spans) for detail queries.\n")
        ...
        async with websockets.connect(BACKEND_WS) as websocket:
            ...
            async with httpx.AsyncClient() as client:
                for i in range(50):
                    ...
                    # Post the span
                    post_resp = await client.post(f"{BACKEND_HTTP}/spans/", json=payload)
                    ...
                    while not ws_received:
                        ...
                        if msg.get("type") == "trace_updated" and msg.get("trace", {}).get("id") == str(uuid.UUID(test_trace_id)):
    ...
    p99_list = statistics.quantiles(list_latencies, n=100)[98] if len(list_latencies) >= 100 else (max(list_latencies) if list_latencies else 0)

    avg_detail = statistics.mean(detail_latencies) if detail_latencies else 0
    p99_detail = statistics.quantiles(detail_latencies, n=100)[98] if len(detail_latencies) >= 100 else (max(detail_latencies) if detail_latencies else 0)

    avg_ws = statistics.mean(ws_delays) if ws_delays else 0
    p99_ws = statistics.quantiles(ws_delays, n=100)[98] if len(ws_delays) >= 100 else (max(ws_delays) if ws_delays else 0)
