...
async def run_benchmark(num_spans=1000, concurrency=5):
    print("=== Chronos Ingestion Benchmark ===")
    ...
    payloads = []
    trace_ids = [
        uuid.uuid4().hex for _ in range(num_spans // 5)
    ]  # 5 spans per trace average
    services = [
        "frontend",
        "gateway",
        "order-service",
    ...
    async with httpx.AsyncClient(limits=limits, timeout=10.0) as client:
        tasks = [send_span(client, sem, payload, latencies) for payload in payloads]
    ...
    if not latencies:
        print(
            "Error: All benchmark requests failed. Make sure the backend server is running on port 8000!"
        )
        return
    ...
    print("\n=== Benchmark Results ===")
    print(
        f"Total Telemetry Spans:     {successful_requests} / {num_spans} successfully ingested"
    )
    ...
    if memory_before and memory_after:
        ...
        print(
            f"Memory Delta Growth:       {max(0.0, memory_after - memory_before):.2f} MB"
        )
